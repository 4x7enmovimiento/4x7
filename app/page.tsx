...
export default function Home() {
  ...
  const familyCheckInData = useMemo(() => {
    ...
    const isMemberMatch = (m: any, checkName: string, checkNick: string, checkEmail?: string) => {
      ...
      if (cEmail) {
        ...
        if (m.phone && (m.phone.toLowerCase() === cEmail || m.phone.replace(/\D/g, "") === cEmail.replace(/\D/g, ""))) return true;
      }

      const normCName = normalize(checkName);
      const normCNick = normalize(checkNick);

      const normMFull = normalize(m.fullName);
      const normMName = normalize(m.name);
      const normMNick = normalize(m.nickname);

      // Nickname matching ignoring emojis (ej. "Ely 🌸" con "Ely")
      if (normCNick && (normMNick === normCNick || normMName === normCNick)) {
        return true;
      }
    ...
    // Dynamically update or add logged in user
    if (session?.user?.name) {
    ...
    // Deduplicación estricta: ninguna persona se repite
    ...
    return uniqueMembers.map((m) => {
      const normNick = normalize(m.nickname);
      ...
      const isRegistered = Boolean(
        ...
        (familyProfiles && (
          ...
          (normNick && familyProfiles[normNick]?.objective) ||
          (m.email && familyProfiles[m.email.toLowerCase()]?.objective)
        )) ||
  ...
  const buildWhatsAppGroupMessage = (
    member: { name: string; nickname?: string; workouts: number; relation: string; activity?: string },
    mode: "carrilla" | "animos" = "animos"
  ) => {
    ...
    if (mode === "animos") {
      if (member.workouts >= 7) {
        ...
        options = [
          `🎟️🔥 ¡A UN SOLO DÍA DE LA GLORIA @${nick}! Llevas 3/4. ¡Hoy se sella esa meta y nadie te quita la rifa! 🏆💪`,
          `⚡🙌 ¡Recta final @${nick}! Estás a solo 30 minutitos de coronar tu semana 4×7. ¡Con toda la energía hoy! 🚀✨`,
          `🌟👊 ¡Qué orgullo de constancia @${nick}! 3 check-ins impecables. ¡A cerrar con broche de oro la meta! 👏🔥`,
          `💥🎉 ¡Ya casi lo logras @${nick}! Un solo entrenamiento más para el 4/4 perfecto. ¡A romperla hoy! 🏋️‍♂️✨`,
          `👟⚡ ¡Vamos @${nick}! La meta está a la vuelta de la esquina (3/4). ¡Todo el grupo te está aplaudiendo! 👏🔥`,
        ];
      } else if (member.workouts === 2) {
        options = [
          `⚡🎯 ¡A mitad del camino @${nick}! Llevas 2 de 4 con pura disciplina. ¡Hoy sacamos el tercero! 👏🔥`,
          `💪🔥 ¡Qué buena racha @${nick}! 2 días listos, ya se siente ese boleto a la rifa. ¡A no aflojar el paso! 🎟️✨`,
          `🚀🙌 ¡Puro poder 4×7 @${nick}! Mitad de semana dominada. ¡El equipo confía en ti para la meta completa! 👊`,
          `🌟👟 ¡Vas excelente @${nick}! 2/4 completados, ¡métele ganas hoy y deja la meta casi lista! 🏃‍♂️⚡`,
          `✨💪 ¡Constancia pura @${nick}! 2 días adentro. ¡30 minutitos hoy y te pones a tiro de la gloria! 🔥`,
        ];
      } else if (member.workouts === 1) {
        options = [
          `🔥👏 ¡Excelente arranque @${nick}! Ya diste el paso más difícil, ¡hoy toca el segundo para afianzar el ritmo! 💪⚡`,
          `🚀✨ ¡Bien jugado @${nick}! 1 check-in adentro. ¡Sigue con esa vibra ganadora y asegura el 2/4 hoy! 👊🔥`,
          `💪🌟 ¡Gran constancia @${nick}! Un entrenamiento a la vez, ¡vamos por ese segundo día en familia! 🏃‍♀️💨`,
          `⚡🎯 ¡Eso es actitud @${nick}! Ya rompiste el hielo con tu primer día, ¡a mantener la chispa encendida! 👏🔥`,
          `👟✨ ¡Bien hecho @${nick}! El primero ya está en la bolsa. ¡Hoy con todo por el segundo! 💥🏃‍♂️`,
        ];
      } else {
        // 0 de 4 entrenamientos
        options = [
          `⚡💪 ¡Venga @${nick}! Hoy es el mejor día para arrancar tu 4×7, ¡30 minutitos y estamos juntos en la meta! 🚀✨`,
          `👟🔥 ¡Tú puedes @${nick}! Toda la familia te echa porras, ¡ponte los tenis y saca el primero hoy! 🏃‍♂️💥`,
          `🌟👊 ¡Ánimo @${nick}! El primer paso es el que cuenta, ¡a darle con todo el corazón que sí se puede! 👏✨`,
          `💥🚀 ¡Poder 4×7 @${nick}! Un entrenamiento hoy y te subes a la tabla con el equipo. ¡Vamos con todo! 💪🔥`,
          `✨💪 ¡Aquí nadie se queda atrás @${nick}! Tómate tu agua, pon tu música favorita y ¡a mover el cuerpo! 🎵🏃‍♀️`,
          `🏋️‍♀️🎉 ¡Vamos con todo @${nick}! Tu salud y energía van primero. ¡Regálate esos 30 minutos de gloria hoy! ⚡✨`,
        ];
      }
    } else {
      // Carrilla
      if (member.workouts >= 7) {
        ...
        options = [
          ...
          `🥇🙌 @${nick} poniendo el ejemplo y burlándose de la zona de tacos (4/4). ¿Quién más le sigue el paso hoy? 👏✨`,
          `🎟️🔥 @${nick} ya está adentro de la tómbola del mes. ¡El resto a sudar la camiseta o sacar la cartera! 🌮😂`,
        ];
      } else if (member.workouts === 3) {
        options = [
          `🦘🔥 ¡A UN DÍA DE SALVARTE! @${nick} lleva 3 de 4. ¡Un solo entreno más y boleto a la rifa en mano! 🎟️💪`,
          `⚡🙌 ¡No te me quedes en la orilla @${nick}! Vas 3/4, ¡si hoy no entrenas te va a costar caro el domingo! 🚀✨`,
        ...
        options = [
          ...
          `🌮💨 @${nick} en zona caliente (1/4). ¡Hoy toca sudar la camiseta o patrocinar la taquiza! 💸✨`,
        ];
      } else {
        // 0 de 4 entrenamientos
        options = [
  ...
  const handleSendWhatsApp = (
    ...
    const message = buildWhatsAppGroupMessage(member, mode);
    ...
    const motivationPost: FeedPost = {
      id: Date.now(),
      userId: session?.user.id || 1,
      userName: (session?.user?.name ? session.user.name.split(" ")[0] : "") || currentUserNick || "Familiar",
      caption: `📣 ${mode === "carrilla" ? "Carrilla" : "Ánimos"} al Grupo para ${displayName}:\n"${message}"`,
      evidenceUrl: null,
      createdAt: new Date().toISOString(),
      activityType: mode === "carrilla" ? "Carrilla Familiar 🔥" : "Ánimos Familiares ⚡",
      durationSeconds: null,
      distanceMeters: null,
      steps: null,
      calories: null,
    ...
    notify(
      mode === "carrilla"
        ? `🔥 Carrilla para ${displayName} copiada. Selecciona el grupo en WhatsApp.`
        : `⚡ Ánimos para ${displayName} copiados. Selecciona el grupo en WhatsApp.`
    );
  };
                ...
                <div className="row-right-actions">
                  {!isCurrentUser && (
                    <>
                      <button
                        type="button"
                        className="whatsapp-motivate-btn animos"
                        onClick={() => handleSendWhatsApp(member, "animos")}
                        title={`Enviar ánimos a ${member.nickname || member.name} al WhatsApp familiar`}
                      >
                      ...
                      <button
                        type="button"
                        className="whatsapp-motivate-btn carrilla"
                        onClick={() => handleSendWhatsApp(member, "carrilla")}
                        title={`Enviar carrilla divertida a ${member.nickname || member.name} al WhatsApp familiar`}
                      >
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
                        </svg>
  ...
  const speakText = (id: string, rawText: string) => {
    ...
    // Limpiar formato markdown, viñetas y emojis para una lectura natural y fluida
    ...
    if (chosenVoice) {
      ...
      utterance.lang = chosenVoice.lang || "es-MX";
    } else {
